...
ratio = 0.08;                  %Delta_rho/rho_mean (Used for Statistical Variation)
...
%%% Method==1 Load Control Method || 
%%% Method==2 Displacment Control Method ||
%%% Method==3 Arc Length Control Method ||
%%% Method==4 Generalised Displacement Control Method ||
Method = 3;

%%% Method==1 Load Control Method
incremental_Load_parameter = 5000;
...
% Storing for GDCM
eta=1;
incremental_Arc_Length_parameter = 2000;

%%% Method==4 Generalised Displacement Control Method ||
% Storing for GDCM
Delta_u_f_1 = zeros(p.Num_Nodes,1);
incremental_Load_parameter_GDCM = 8000;


if Method==1
    increment=incremental_Load_parameter;
elseif Method==2
    increment=incremental_Disp_parameter;
elseif Method==3
    increment=incremental_Arc_Length_parameter;
elseif Method==4
    increment=incremental_Load_parameter_GDCM;
end
...
while i<n+1
    ...
    Ref_Load_vector(:,i) = Find_force_unit_vector(p,p.Element_G,p.Node_coordinate);
    
    if Method==1
        u_0 = u(:,i-1);
        F_0 = F_ext(:,i-1);
        [u(:,i),Force(:,i),F_ext(:,i),R(:,i),Shear_Stress(:,i),Shear_Strain(:,i),Shear_Plastic_Strain(:,i),Element_G_array(:,i),Colour(:,i)] = Load_Control(increment,u_0,F_0,Ref_Load_vector(:,i),p,Shear_Stress(:,i-1),Shear_Strain(:,i-1),Shear_Plastic_Strain(:,i-1),Element_G_array(:,i-1),Colour(:,i-1),tol,max_iter);
    elseif Method==2
        u_0 = u(:,i-1);
        F_0 = F_ext(:,i-1);
        [u(:,i),Force(:,i),F_ext(:,i),R(:,i),Shear_Stress(:,i),Shear_Strain(:,i),Shear_Plastic_Strain(:,i),Element_G_array(:,i),Colour(:,i)] = Displacement_Control(increment,u_0,F_0,Ref_Load_vector(:,i),p,Shear_Stress(:,i-1),Shear_Strain(:,i-1),Shear_Plastic_Strain(:,i-1),Element_G_array(:,i-1),Colour(:,i-1),tol,max_iter);
    elseif Method==3
        u_0 = u(:,i-1);
        F_0 = F_ext(:,i-1);
        [u(:,i),Force(:,i),F_ext(:,i),R(:,i),Shear_Stress(:,i),Shear_Strain(:,i),Shear_Plastic_Strain(:,i),Element_G_array(:,i),Broken_Elements(:,i),Colour(:,i)] = Arc_Length_Control(increment,u_0,F_0,Ref_Load_vector(:,i),p,Shear_Stress(:,i-1),Shear_Strain(:,i-1),Shear_Plastic_Strain(:,i-1),Element_G_array(:,i-1),Broken_Elements(:,i-1),Colour(:,i-1),tol,max_iter,eta);
    elseif Method==4
        u_0 = u(:,i-1);
        F_0 = F_ext(:,i-1);
        [u(:,i),Delta_u_f_1,Force(:,i),F_ext(:,i),R(:,i),Shear_Stress(:,i),Shear_Strain(:,i),Shear_Plastic_Strain(:,i),Element_G_array(:,i),Colour(:,i)] = GDCM(increment,u_0,F_0,Delta_u_f_1,Ref_Load_vector(:,i),p,Shear_Stress(:,i-1),Shear_Strain(:,i-1),Shear_Plastic_Strain(:,i-1),Element_G_array(:,i-1),Colour(:,i-1),tol,max_iter,i);
    end
...
%     K = Global_Stiff_Matrix(p,Element_G_array(:,i),p.Node_coordinate);    
%     if det(K(p.active_Dof_force,p.active_Dof_force))==0
%         Shear_Stress(:,i:end)=[];
%         Shear_Strain(:,i:end)=[];
%         u(:,i:end)=[];        %Displacement at which crack happens
%         Force(:,i:end)=[];    %Force at the time of crack
%         Ref_Load_vector(:,i:end)=[];
%         F_ext(:,i:end)=[];
%         Colour(:,i:end)=[];
%         Shear_Plastic_Strain(:,i:end)=[];
%         lambda(i:end)=[];
%         Element_G_array(:,i:end)=[];
%         R(:,i:end)=[];
%         Broken_Elements(i:end,:)=[];
%         break;
%     end
    ...
    if size(I3_break,1)>=(size(Shear_Stress(:,i),1)/(Number_of_Springs_in_row)) %&& any(Shear_Strain(:,i) >= epsilon_u)
...
%     legend(legends{k1},'Location','northwest');
%     grid on;
%     pause(1);
%     clf(f)
% end
...
% figure(20)
% hold on
% plot(t, Shear_Stress(5,:));
% hold on
% plot(t, Shear_Stress(11,:));
% hold on
% plot(t, Shear_Stress(17,:));
